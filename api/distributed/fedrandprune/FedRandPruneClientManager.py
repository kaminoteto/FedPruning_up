...
class FedRandPruneClientManager(ClientManager):
    def __init__(self, args, trainer: FedRandPruneTrainer, comm=None, rank=0, size=0, backend="MPI"):
        super().__init__(args, comm, rank, size, backend)
        self.trainer:FedRandPruneTrainer = trainer
    ...
    def __train(self):
        ...
        weights, _, local_sample_num = self.trainer.train(mode = self.mode, round_idx=self.round_idx)
