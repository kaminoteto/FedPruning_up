...
from api.distributed.fedrandprune.FedRandPruneAggregator import FedRandPruneAggregator
...
class FedRandPruneServerManager(ServerManager):
    def __init__(self:ServerManager, args, aggregator, comm=None, rank=0, size=0, backend="MPI", is_preprocessed=False, preprocessed_client_lists=None):
        super().__init__(args, comm, rank, size, backend)
        self.args = args
        self.aggregator: FedRandPruneAggregator = aggregator
        ...
        # mode 0, the server send both weight and mask to clients, received the weight, perform weight aggregation
