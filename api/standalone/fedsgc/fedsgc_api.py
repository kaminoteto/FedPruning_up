...
class FedSGCAPI(object):
    def __init__(self, dataset, device, args, model_trainer):
        ...
        self.mask = self._generate_mask(self.model_trainer.model, self.args.lambda_param)
    ...
    def _generate_mask(self, model, lambda_param):
        mask = {}
        for name, param in model.named_parameters():
            if 'weight' in name:
                mask[name] = (param.abs() > lambda_param).float()
        return mask

    def apply_weight_mask(self, model, mask):
        with torch.no_grad():
            for name, param in model.named_parameters():
                if 'weight' in name:
                    param.mul_(mask)
        logging.info("Weight mask applied to model.")
    ...
    def train(self):
        ...
        for round_idx in range(self.args.comm_round):
            ...
            if round_idx % self.args.delta_T == 0 and round_idx < self.args.T_end :
                mode = 2 # when mode is 2, it will perform pruning and growing
            else:
                mode = 0
            ...
            if round_idx == self.args.A_epochs:
                logging.info("Applying weight mask after specified steps.")
                self.apply_weight_mask(model, self.mask)
    ...
    def test_on_server_for_all_clients(self, round_idx, dataset):
        test_num_samples = []
        test_tot_corrects = []
        test_losses = []

        metrics = self.model_trainer.test(dataset, self.device, self.args)
