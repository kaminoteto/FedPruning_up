...
class MyModelTrainer(ModelTrainer):
    ...
    # Set the entire model to evaluation mode
        ...
        # Manually set BN layers to training mode
        for module in model.modules():
            if isinstance(module, torch.nn.BatchNorm2d) or isinstance(module, torch.nn.BatchNorm1d):
                module.train()

        # Perform a forward pass to update BN running statistics
        with torch.no_grad():  # Disable gradient computation
            for batch_idx, batch in enumerate(train_data):
                tokenized = self.tokenizer(batch['text'], padding=True, return_tensors='pt', max_length=256, truncation=True)['input_ids'].to(device)
                model.zero_grad()
                logits, loss = model(tokenized, tokenized)
        # Collect updated BN layer parameters and running statistics
        bn_parameters = {}
        for name, param in model.named_parameters():
            if "bn" in name:
                bn_parameters[name] = param.data.cpu().clone()

        # # Collect running statistics (running_mean and running_var)
        # for name, module in model.named_modules():
        #     if isinstance(module, torch.nn.BatchNorm2d) or isinstance(module, torch.nn.BatchNorm1d):
        #         bn_parameters[f"{name}.running_mean"] = module.running_mean.cpu().clone()
        #         bn_parameters[f"{name}.running_var"] = module.running_var.cpu().clone()
